...
# Game Title
pygame.display.set_caption("Childhood")
...
def text_screen(text, color, x, y):
    ...
    #blit is used to copy the object from one to another
    window_of_the_game.blit(screen_text, [x,y])


def plot_snake(widow_of_the_game, color, snake_list, snake_size):
    for x,y in snake_list:
        pygame.draw.rect(widow_of_the_game, color, [x, y, snake_list, snake_size])
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            text_screen("Game Over! Press Enter To Continue", food, 100, 250)
            ...
            if snake_x<0 or snake_x>screen_width-20 or snake_y<50 or snake_y>screen_height-20:
                game_over = True
            plot_snake(window_of_the_game, grey, snake_list, snake_size)
        pygame.display.update()
        clock.tick(fps)
    pygame.quit()
    quit()
